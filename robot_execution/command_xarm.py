...
# Utility function to convert Joint State into Commands (for the bus servos).
    def convert_joint_state_to_commands(self,jt_all):
        # Check for NaN values first - if any joint angle is NaN, don't process
        if any(np.isnan(jt_all)):
            self.get_logger().warning('NaN detected in joint angles. Using current servo positions.')
            return self.cmd_all  # Return current commands instead
        ...
        # Convert to integers - add additional NaN check just in case
        try:
            cmd_all = [int(cmd) if not np.isnan(cmd) else self.cmd_all[i] 
                      for i, cmd in enumerate([cmd_00,cmd_01,cmd_02,cmd_03,cmd_04,cmd_05,cmd_06])]
        except:
            # If anything still fails, return current commands
            self.get_logger().error('Error converting commands to integers. Using current positions.')
            return self.cmd_all
